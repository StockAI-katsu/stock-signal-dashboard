...
def parse_tradingview_csv(uploaded_files):
    """トレーディングビューの変則的なアラートログCSVから有効な銘柄・日時・手法を正しく抽出する"""
    all_new_rows = []
    ...
    for uploaded_file in uploaded_files:
        try:
            ...
            # 各列を文字列として安全に扱うため、全列名とデータを走査
            # トレーディングビューのCSV特有の構造に対応（行ごとに「ティッカー」「名前」「名称」「日時」が分散しているケースを考慮）
            for _, row in df_raw.iterrows():
                row_str = " ".join([str(val) for val in row.values if pd.notna(val)])
                
                # ティッカー（TSE:XXXX など）が含まれている行を探す
                ticker_val = ""
                for col in df_raw.columns:
                    val_str = str(row.get(col, ""))
                    if "TSE:" in val_str or "prime:" in val_str.lower() or "tse" in val_str.lower():
                        ticker_val = val_str
                        break
                
                # 行内に証拠となるティッカーや銘柄コードらしきものがない場合はスキップ
                if not ticker_val:
                    # 代替として行全体からコードを探す
                ...
                for col in df_raw.columns:
                    val_str = str(row.get(col, ""))
                    if "T" in val_str and ("202" in val_str or "203" in val_str): # ISO形式などの日付をキャッチ
                        date_part = val_str.split("T")[0].split(" ")[0]
                        break

                # 手法・メッセージの判定
                name_val = str(row.get("名前", row.get("名称", "")))
                ...
                # コードのクレンジング
                numeric_code = "".join([c for c in ticker_val if c.isdigit()])
                if len(numeric_code) >= 4:
                    numeric_code = numeric_code[-4:].zfill(4)
                else:
                    continue
...
if uploaded_files:
    if st.sidebar.button("🔄 マスター台帳に統合・更新する"):
        ...
        if not new_df.empty:
            ...
            st.sidebar.warning("CSVから有効なシグナルデータを検出できませんでした。フォーマットをご確認ください。")
