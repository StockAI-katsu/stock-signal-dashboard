    ...
    if os.path.exists(EXCEL_MASTER_FILE):
        try:
            ...
            for _, row in df_ex.iterrows():
                ...
                sector_raw = str(row.get("33業種区分", row.get("セクター", row.get("33業種", row.get("業種", "未分類")))))
                ...
                sector_dict[code] = sector_raw if sector_raw != "nan" and sector_raw != "" else "未分類"
...
def load_master():
    """Googleドライブ同期対象のExcelファイル（master_signalsシート）からマスターデータを読み込む"""
    if os.path.exists(MASTER_EXCEL_OUT):
        try:
            # 確実に "master_signals" シートから読み込む
            df = pd.read_excel(MASTER_EXCEL_OUT, sheet_name="master_signals", dtype={"コード": str})
            for col in ALL_COLUMNS:
                if col not in df.columns:
                    df[col] = None
            return df
        except Exception as e:
            # 万が一シートが存在しない場合のフォールバック（初回作成時など）
            try:
                xls = pd.ExcelFile(MASTER_EXCEL_OUT)
                if xls.sheet_names:
                    df = pd.read_excel(MASTER_EXCEL_OUT, sheet_name=xls.sheet_names[0], dtype={"コード": str})
                    for col in ALL_COLUMNS:
                        if col not in df.columns:
                            df[col] = None
                    return df
...
def save_master(df):
    """マスターデータをGoogleドライブ同期対象のExcelファイルの "master_signals" シートへ保存・永続化する"""
    try:
...
def parse_tradingview_csv(uploaded_files):
    """トレーディングビューCSVをパースする（Excelの情報を即座に紐付け）"""
    all_new_rows = []
...
def fetch_stock_data_for_master(df):
    """株価データの取得と、Excelマスターからの情報再同期を行う"""
    if df.empty:
        return df
...
st.markdown(
    f"Excelファイル (`{EXCEL_MASTER_FILE}`) の銘柄データをベースに、シグナルと20営業日分の値動きを自動追跡し、`{MASTER_EXCEL_OUT}` の **`master_signals` シート** に永続保存します。"
)
...
if uploaded_files:
    if st.sidebar.button("🔄 マスター台帳に統合・更新する"):
        ...
        if not new_df.empty:
            ...
            master_df = combined
            st.sidebar.success(f"マスター台帳を更新し、Excelの master_signals に保存しました！（総件数: {len(master_df)}件）")
            st.rerun()

if not master_df.empty:
    if st.sidebar.button("🚀 株価データ・市場情報一括自動取得・更新"):
        with st.spinner("株価データおよびExcel上のセクター・市場情報を同期中..."):
            updated_df = fetch_stock_data_for_master(master_df)
            save_master(updated_df)
            master_df = updated_df
            st.sidebar.success("データとExcel情報の同期・保存が完了しました！")
            st.rerun()
    ...
    with selected_tabs[idx]:
        if idx == 0:
            ...
            if all_candidates:
                ...
                st.success(f"条件に合致する候補銘柄が **{len(combined_candidates_df)}件** 見つかりました！")
            ...
            if methods and not master_df.empty:
                for m in methods:
                    ...
                    sectors = [s for s in m_df["セクター"].dropna().unique() if str(s).strip() != "" and str(s) != "nan"]
                    if sectors:
            ...
            if not candidates_df.empty:
                st.success(f"条件に合致する候補銘柄が **{len(candidates_df)}件** 見つかりました！")
            ...
            st.markdown("#### 🏢 セクター別最適戦略")
            sectors = [s for s in target_df["セクター"].dropna().unique() if str(s).strip() != "" and str(s) != "nan"]
            if sectors:
                for sector in sectors:
