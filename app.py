...
def load_excel_stock_master():
    """Excelの『銘柄データ』シートからコードと銘柄名・市場・セクターの辞書を作成する"""
    name_dict = {}
    market_dict = {}
    sector_dict = {}
    if os.path.exists(EXCEL_MASTER_FILE):
        try:
            ...
            for _, row in df_ex.iterrows():
                ...
                sector_raw = str(row.get("セクター", row.get("業種", "")))
...
def load_master():
    try:
        # スプレッドシートの「master_signals」シートから読み込む
...
def fetch_stock_data_for_master(df):
    ...
    for idx, row in df.iterrows():
        ...
        if sector_val == "未分類" or pd.isna(sector_val):
            try:
