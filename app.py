...
def fetch_stock_data_for_master(df):
    ...
    for idx, row in df.iterrows():
        ...
        raw_code = str(row.get("コード", "")).replace(".T", "").replace("TSE:", "").strip().zfill(4)
        if not raw_code or raw_code == "0000":
            # ティッカー列からコードを再抽出試行
            ticker_str = str(row.get("ティッカー", ""))
            matches = re.findall(r'([1-9][0-9]{3})', ticker_str)
            if matches:
                raw_code = matches[-1].zfill(4)
            else:
                debug_messages.append(f"行 {idx}: コードが取得できません (コード値: {row.get('コード')})")
                continue

        yf_code = f"{raw_code}.T"
        start_date_str = str(row.get("発生日", "")).split("T")[0].split(" ")[0]

        try:
            start_date = datetime.strptime(start_date_str, "%Y-%m-%d")
        except Exception as ex:
            debug_messages.append(f"行 {idx} ({raw_code}): 発生日パース失敗 ({start_date_str}) -> {ex}")
            # 発生日が不正な場合は本日の日付をフォールバックとして設定
        ...
        try:
            ...
            if hist.empty:
                debug_messages.append(f"銘柄 {yf_code}: yfinanceでデータが取得できませんでした (期間: {start_date.strftime('%Y-%m-%d')} ~)")
                continue
            ...
            if len(future_days) == 0:
                debug_messages.append(f"銘柄 {yf_code}: 発生日以降の株価データが存在しません (発生日: {start_date_str})")
                continue
            ...
            debug_messages.append(f"銘柄 {yf_code} 処理例外: {e}")
            continue

    progress_bar.empty()
    if debug_messages:
        with st.expander("🔍 株価取得のデバッグログ（クリックして確認）", expanded=True):
...
if uploaded_files:
    if st.sidebar.button("🔄 マスター台帳に統合・更新する"):
        ...
        if not new_df.empty:
            ...
            st.sidebar.warning("CSVから有効なシグナルデータを検出できませんでした。")
