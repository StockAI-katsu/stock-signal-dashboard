...
def fetch_stock_data_for_master(df):
    ...
    sector_map = {
        "Energy": "エネルギー", "Utilities": "電気・ガス", "Industrials": "資本財・インフラ",
        "Technology": "情報通信・テクノロジー", "Financial Services": "金融", "Consumer Cyclical": "消費財（一般）",
        "Consumer Defensive": "消費財（必需）", "Healthcare": "医薬品・ヘルスケア", "Basic Materials": "素材・化学",
...
if not master_df.empty and "手法" in master_df.columns:
    ...
    for method in methods:
        ...
        # 第1位の戦略のイン日数（例: "1営業日後（始値）" から数値を抽出）
        ...
        for _, row in m_df.iterrows():
            ...
            # 簡易的な営業日カウント（土日を除いた日数差を大まかな目安とする）
            # 厳密な営業日計算：発生日から今日までの営業日数を計算
            business_days = pd.bdate_range(start=start_dt, end=today)
            elapsed_business_days = len(business_days) - 1 # 発生日当日を0日目とする場合
            
            # 「明日」がエントリー日ということは、現在の経過営業日数が (target_in_day - 1) 日目である銘柄
            # 例: 1営業日後エントリーが最適なら、発生日当日にシグナルが出て、翌営業日が1日目（明日イン）
...
if not master_df.empty and "手法" in master_df.columns:
    ...
    if sectors:
        ...
        if not sector_strategies.empty:
            st.dataframe(sector_strategies, use_container_width=True)
        else:
...
if not master_df.empty and "手法" in master_df.columns:
    ...
    if methods:
        ...
        def render_table(df_target):
            st.dataframe(df_target[display_cols], use_container_width=True)
