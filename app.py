...
def load_excel_stock_master():
    """Excelの『銘柄データ』シートからコード、銘柄名、市場、セクター(33業種)の辞書を作成する"""
    name_dict = {}
    market_dict = {}
    sector_dict = {}
    ...
    if os.path.exists(EXCEL_MASTER_FILE):
        try:
            ...
            for _, row in df_ex.iterrows():
                ...
                # Excel上のセクター（33業種など）の列名を想定（例：「セクター」や「33業種」など、実際の列名に合わせて調整可能）
                # ここでは代表的な列名候補を安全に取得します
                sector_raw = str(row.get("セクター", row.get("33業種", row.get("業種", "未分類"))))
...
def fetch_stock_data_for_master(df):
    ...
    for idx, row in df.iterrows():
        ...
        # 常に最新のExcelマスターから名前・市場・セクターを正確に反映
...
if not master_df.empty:
    if st.sidebar.button("🚀 株価データ・市場情報一括自動取得・更新"):
        with st.spinner("株価データおよびExcel上のセクター・市場情報を同期中..."):
            ...
            st.sidebar.success("データとExcel情報の同期が完了しました！")
            st.rerun()
